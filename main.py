...
def update():
    if request.method == "POST":
        ...
        getNewDob=request.form["newdob"]
        getNewPlace = request.form["newplace"]
        getNewPincode = request.form["newpincode"]
        con.execute("UPDATE PATIENT SET NAME = '"+getNewname+"',AGE = '"+getNewAge+"',ADDRESS='"+getNewAddress+"',DOB='"+getNewDob+"',PLACE = '"+getNewPlace+"',PINCODE = '"+getNewPincode+"' WHERE MOBILENO = '"+getMobileNO+"' ")
